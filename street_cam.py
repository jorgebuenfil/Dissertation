...
def boxes_to_pixel_units(box_list, image_width, image_height, grid_size):
    """
    Converts the boxes in box list to pixel units assumes box_list is the output from the box output from
...
def get_intersection_over_union(box_1, box_2):
    ...
    :param box_1: array of 4 numbers which are the (x, y) points that define the center
    of the box and the length and width of the box.
    :param box_2: array of 4 numbers which are the (x, y) points that define the center
    of the box and the length and width of the box.
...
def overlay_on_image(display_image, filtered_objects):
    ...
    :param display_image: image on which the inference was run.
    :param filtered_objects: list of lists (as returned from filter_objects()
...
#     [8]:float value that is the googlenet probability
    ...
    :return: True if should go to next image or False if should not.
    """
...
def get_googlenet_classifications(gn_graph, source_image, filtered_objects):
    ...
    :param gn_graph: graph object on which the inference should be executed.
    :param source_image: original image on which the inference was run.  The boxes
    defined by filtered_objects are rectangles within this image and will be used as input for GoogLeNet.
    :param filtered_objects: [IN/OUT] upon input is a list of lists (as returned from filter_objects()
    each of the inner lists represent one found object and contain:
        - string that is network classification ie 'cat', or 'chair' etc.
        - float value for box center X pixel location within source image.
        - float value for box center Y pixel location within source image.
    ...
    Upon output the following 3 values from the GoogLeNet inference will be added to each
    inner list of filtered_objects:
    ...
    :return: Nothing.
    """
    # pad the height and width of the image boxes by this amount to make sure we get the whole object
    # in the image that we pass to GoogLeNet
    WIDTH_PAD = 20
    HEIGHT_PAD = 30
...
def googlenet_inference(gn_graph, input_image):
    ...
    it is assumed that this has been created with allocate graph and the
    googlenet graph file on an open NCS device.

    :param gn_graph: googlenet graph object to use for the inference.
...
def main():
    ...
    #print_info()
    ty_device, gn_device = mv.open_ncs_device(number_of_devices=2)

    # Load tiny YOLO graph from disk and allocate graph via API
    try:
        ty_graph, ty_input_fifo, ty_output_fifo = mv.load_multidevice_graph(ty_device, tiny_yolo_graph_file)
    except:
    ...
    try:
        gn_graph, gn_input_fifo, gn_output_fifo = mv.load_multidevice_graph(gn_device, googlenet_graph_file)
    except:
        print('Error - could not load GoogLeNet graph file')
        mv.close_ncs_device(ty_device, ty_graph)
    ...
    while True:
        for input_video_file in input_video_filename_list:
            ...
            while True:
                ...
                # for an inference
                input_image = cv2.resize(input_image, (TY_NETWORK_IMAGE_WIDTH,
                                                       TY_NETWORK_IMAGE_HEIGHT), cv2.INTER_LINEAR)
                ...
                output, userobj, ty_input_fifo, ty_output_fifo = \
                    mv.infer_multidevice_image(ty_graph, input_image, ty_input_fifo, ty_output_fifo)
    ...
    # clean up tiny yolo
    mv.close_multidevice_ncs_device(ty_device, ty_graph, ty_input_fifo, ty_output_fifo)

    # Clean up googlenet
    mv.close_multidevice_ncs_device(gn_device, gn_graph, gn_input_fifo, gn_output_fifo)
